...
filter: expr { parse_result = $1; $$ = $1; }
      | expr LOGICAL expr { // 创建一个跳转标签结点
                            ...
                            $$ = right_expr;
                            parse_result = right_expr; }
      ;

expr: field COMPARISON constant { struct bpf_syntax_node *node = bpf_syntax_node_new(BPF_SYNTAX_NODE_OPERATOR_COMPARISON, $2);
                                  node->left = $1;
                                  node->right = $3;
                                  $1->parent = node;
                                  $3->parent = node;
...
field: FIELD { struct bpf_syntax_node *node = bpf_syntax_node_new(BPF_SYNTAX_NODE_FIELD, $1);
               $$ = node;}
     ;

constant: NUMBER { struct bpf_syntax_node *node = bpf_syntax_node_new(BPF_SYNTAX_NODE_CONSTANT, $1);
                  $$ = node; }
        ;
